...
public class Artwork implements Comparable<Artwork> {
  ...
  public int compareTo(Artwork otherArtwork){
    if(otherArtwork == null) throw new NullPointerException("otherArt is Null");

    int otherYear = otherArtwork.getYear();
    double otherCost = otherArtwork.getCost();
    String otherName = otherArtwork.getName();

    if(year == otherYear && cost == otherCost && name.equals(otherName)){
      return 0;
    }
    else if(year == otherYear && cost == otherCost){
      return name.compareTo(otherName);
    }
    else if(year == otherYear){
      return (int)(cost - otherCost);
    }
    else if(year < otherYear){ //there is 100% a better way to if else this but... yeah
